...
public class MapItemManager implements Listener {
    ...
    private static boolean give(Player player, SingleMap map) {
        return give(player, createMapItem(map));
    }

    private static boolean give(Player player, PosterMap map) {
    ...
    private static String getMapTitle(PosterMap map, int row, int column) {
        return I.t("{0} (row {1}, column {2})", map.getName(), row + 1, column + 1);
    }

    private static String getMapTitle(PosterMap map, int index) {
